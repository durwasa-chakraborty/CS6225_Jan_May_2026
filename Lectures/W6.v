...
- The [;] Tactical: The compound tactic [T;T'] first performs
 [T] and then performs [T'] on _each subgoal_ generated by [T].
- The [try] Tactical: If [T] is a tactic, then [try T] is a tactic 
that is just like [T] except that, if [T] fails, 
[try T] _successfully_ does nothing at all (rather than failing).
**)

(** Some other useful tactics:
- subst : substitute away _all_ assumptions of the form 
[x = e]or [e = x] (where [x] is a variable).
...
| <{if b then c1 else c2 end}> => if beval st b then ceval_try st c1
   else ceval_try st c2
| <{while b do c end}> => st             (* <--- bogus *)
end.

(**
| <{while b do c end}> => if beval st b then 
                          ceval_try st <{c ; while b do c end}>
                          else st
The above definition would be correct, but Rocq would complain that
the function cannot be proven as terminating.
...
| E_Skip (st : state) :
   st =[<{skip}>]=>c st
| E_Asgn (st : state) (X : string) (a : aexp) (n : nat)
   (Ha: aeval st a = n) :
   st =[<{X := a}>]=>c (X !-> n; st)
| E_Seq (st st' st'': state) (c1 c2 : com) 
  ...
  (Hc2 : st'' =[c2]=>c st') :
   st =[<{c1 ; c2}>]=>c st'
| E_IfTrue (st st' : state) (c1 c2 : com) (b : bexp) 
  (HTrue : beval st b = true)
  (Hc1 : st =[c1]=>c st') :
   st =[<{if b then c1 else c2 end}>]=>c st'
| E_IfFalse (st st' : state) (c1 c2 : com) (b : bexp) 
   (HFalse : beval st b = false) 
   (Hc2 : st =[c2]=>c st') :
   st =[<{if b then c1 else c2 end}>]=>c st'
| E_WhileFalse (st : state) (c : com) (b : bexp) 
   (HFalse : beval st b = false) :
   st =[<{while b do c end}>]=>c st
| E_WhileTrue (st st' st'': state) (c : com) (b : bexp) 
   (HTrue: beval st b = true) 
   (Hc: st =[c]=>c st'') 
   (Hr: st'' =[<{while b do c end}>]=>c st') :
   st =[<{while b do c end}>]=>c st'
...
- intros st2 E2. inversion E2. apply IHE1_1 in Hc1. 
  rewrite <- Hc1 in Hc2. apply IHE1_2 in Hc2. apply Hc2. 
- intros st2 E2. inversion E2. apply IHE1. apply Hc1.
  rewrite HTrue in HFalse. discriminate HFalse.
- intros st2 E2. inversion E2. rewrite HTrue in HFalse. discriminate HFalse. 
  apply IHE1. apply Hc2.
-  intros st2 E2. inversion E2. reflexivity.
  rewrite HTrue in HFalse; discriminate HFalse.
- intros st2 E2. inversion E2. subst. rewrite HTrue in HFalse; discriminate HFalse.
  apply IHE1_1 in Hc. rewrite <- Hc in Hr. apply IHE1_2 in Hr. apply Hr.
Qed.
...
Lemma t_update_eq : forall (A : Type) (m : total_map A) x v,
  (x !-> v ; m) x = v.
Proof. intros. unfold update. destruct (x =? x)%string eqn:E.
 - reflexivity.
 - rewrite String.eqb_refl in E. discriminate E.
Qed.
...
Theorem plus2_spec : forall st n st',
  ...
  st =[ plus2 ]=>c st' ->
  st' X = n + 2.
Proof. intros st n st' H E. inversion E. subst. rewrite t_update_eq. 
  reflexivity.
Qed.
...
Theorem XtimesYinZ_spec : forall st n m st',
  ...
  st =[ XtimesYinZ ]=>c st' ->
  st' Z = n * m.
Proof. intros. inversion H1. subst. rewrite t_update_eq. reflexivity.
Qed.
...
Proof.
  ...
  - injection Heqloopdef as Hb Hc. subst. discriminate HFalse.
...
Theorem no_whiles_terminating: forall (c : com) (st : state),
  no_whiles c = true -> exists (st' : state), st =[ c ]=>c st'.
Proof. intros c st H. generalize dependent st. induction c. 
- intros. exists st. constructor.
- intros. exists (x !-> (aeval st a); st). constructor. reflexivity.
- intros. simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
  specialize IHc1 with (st := st). apply IHc1 in H1. 
  destruct H1 as [st'' H1]. specialize IHc2 with (st := st'').
  apply IHc2 in H2. destruct H2 as [st' H2]. exists st'. 
  apply E_Seq with (st'' := st'').
  assumption. assumption.
- intros. simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
  specialize IHc1 with (st := st). specialize IHc2 with (st := st).
  apply IHc1 in H1.  apply IHc2 in H2. destruct H1 as [st1 H1].
  destruct H2 as [st2 H2]. destruct (beval st b) eqn:E.
    -- exists st1. apply E_IfTrue. assumption. assumption.
    -- exists st2. apply E_IfFalse. assumption. assumption.
